...
```text
{IDP_URL}/?client_id={CLIENT_ID}&redirect_uri={REDIRECT_URI_ENCODED}&response_type=id_token&state={STATE}
```

Ejemplo local:

```text
http://localhost:5173/?client_id=mock-client-app&redirect_uri=http%3A%2F%2Flocalhost%3A3000&response_type=id_token&state=random_state_string
```
...
| `state` | Recomendado | Valor opaco generado por el Launcher para correlacionar solicitud y respuesta. |
...
## 3.1 Login por enlace de correo (dos pestañas)
...
sequenceDiagram
    ...
    L->>I: Redireccion con client_id, redirect_uri y state
    ...
    I->>L: Redireccion a redirect_uri#id_token=...&state=...
    L->>L: Lee hash, valida state y procesa token
...
```text
http://localhost:3000/#id_token=<JWT>&state=random_state_string
```

Responsabilidades del Launcher al recibir el retorno:

1. Leer `window.location.hash`.
2. Extraer `id_token` y `state`.
3. Comparar `state` contra el valor generado antes de redirigir al IdP.
4. Validar el token segun la politica del cliente.
5. Crear la sesion local del Launcher.
6. Limpiar el fragmento de URL para no dejar el token visible en historial o UI.
...
if (idToken && returnedState === sessionStorage.getItem("oidc_state")) {
...
```text
http://localhost:3000/#error=login_required&error_description=Sesión%20no%20activa%20en%20el%20IdP&state=refresh
```
...
| --- | --- | --- |
| Hay sesion activa en el IdP | `#id_token=<JWT>&state=refresh` | Reemplazar el token local. |
| No hay sesion activa | `#error=login_required&...&state=refresh` | Pedir login interactivo o cerrar sesion. |
| Error tecnico | `#error=server_error&...&state=refresh` | Mostrar mensaje controlado y permitir reintento. |
